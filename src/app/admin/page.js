...
function AdminContent({ me }) {
  ...
  const handleCreate = async () => {
    if (!form.email || !form.password) {
      showToast('メールアドレスとパスワードは必須です', 'error'); return
    }
    setSaving(true)
  ...
  const handleRoleChange = async (userId, newRole) => {
    if (userId === me.id) {
      showToast('自分のロールは変更できません', 'error'); return
    }
  ...
  const handleDelete = async (userId, userName) => {
    if (userId === me.id) {
      showToast('自分自身は削除できません', 'error'); return
    }
  ...
  return (
    <>
      <nav style={{
        background:'#fff', borderBottom:'.5px solid rgba(0,0,0,.12)',
            ...
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
              <thead>
                <tr style={{ background:'#f5f4f0' }}>
                  {['名前','メールアドレス','ロール','会社名','登録日','操作'].map(h => (
                    <th key={h} style={{
                      padding:'10px 14px', textAlign:'left',
                      fontWeight:500, fontSize:12, color:'#5a5a56',
                      borderBottom:'.5px solid rgba(0,0,0,.15)', whiteSpace:'nowrap'
                    }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {users.map(u => (
                  <tr key={u.id} style={{ borderBottom:'.5px solid rgba(0,0,0,.08)' }}>
                    ...
                    <td style={{ padding:'12px 14px', color:'#5a5a56' }}>{u.email}</td>
                    <td style={{ padding:'12px 14px' }}>
                      <select
                        ...
                        disabled={u.id === me.id}
                        style={{
                          border:'.5px solid rgba(0,0,0,.2)', borderRadius:6,
                          padding:'4px 8px', fontSize:12,
                          background: u.role==='admin'?'#fdecea':u.role==='staff'?'#e6f1fb':'#eaf3de',
                          color: u.role==='admin'?'#c0392b':u.role==='staff'?'#185fa5':'#2d6a4f',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <button onClick={() => setModal(false)} style={{
              ...
              fontSize:20, cursor:'pointer', color:'#9a9a94'
            }}>×</button>
            ...
            {[
              { label:'名前', key:'name', placeholder:'山田太郎', type:'text' },
              { label:'メールアドレス *', key:'email', placeholder:'example@email.com', type:'email' },
              { label:'初期パスワード *', key:'password', placeholder:'8文字以上', type:'password' },
              { label:'会社名', key:'company', placeholder:'〇〇行政書士事務所', type:'text' },
            ].map(f => (
              <div key={f.key} style={{ marginBottom:14 }}>
                <label style={{
                  display:'block', fontSize:11, color:'#5a5a56',
            ...
            <div style={{ display:'flex', justifyContent:'flex-end', gap:8 }}>
              <button onClick={() => setModal(false)} style={{
                padding:'9px 16px', border:'.5px solid rgba(0,0,0,.2)',
